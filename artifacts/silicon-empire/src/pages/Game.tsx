...
function PhaseTicker() {
  ...
  const labels = ["intel", "event", "action", "resolution"];
  const phaseIndex = labels.indexOf(phase);
        ...
        {labels.map((p, i) => (
          <div key={p} className="flex items-center gap-1">
            <div className={`flex items-center gap-1.5 px-2 py-1 rounded text-[10px] uppercase tracking-wider font-semibold transition-all ${
              ...
              {p === "intel" ? "Intel" : p === "event" ? "Event" : p === "action" ? "Action" : "Resolution"}
            </div>
            {i < labels.length - 1 && (
              <div className={`w-4 h-px ${i < phaseIndex ? "bg-primary/40" : "bg-border"}`} />
            )}
          </div>
        ))}
      </div>
